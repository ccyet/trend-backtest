...
- `被拦截信号`：表示已真实触发，但被 MA / ATR / 板块均线 / 导入指标过滤拦下
- `成交模拟失败信号`：表示过滤已通过，但在 stop-entry 或后续成交模拟阶段失败
- `决策链速览`：展示成交交易的开仓原因、成交方式、离场原因和分批 fill 明细
- `信号轨迹下钻`：逐条展示 setup 原因、trigger 原因、各过滤器 pass/fail 和最终是否形成平仓交易
...
- 旧版整笔退出主要用于未启用分批的场景
- “次级固定止盈”属于旧版整笔退出链，只对未启用分批止盈的整笔仓位生效
- 次级固定止盈顺序位于：全仓止损、分批退出、板块均线离场、导入指标整笔离场之后
- 在旧版整笔退出链内，它排在利润回撤、均线离场、ATR 跟踪止盈之后
- 一旦次级固定止盈触发，会按固定止盈价对剩余全仓一次性平掉，不会和分批 `fixed_tp` 叠加执行
