...
def test_fetch_bars_for_timeframe_routes_1m_and_5m_to_tdx_quant(monkeypatch):
    calls: list[tuple[str, str]] = []

    monkeypatch.setattr(
        upd.TdxQuantProvider,
        "fetch_bars",
        staticmethod(lambda **kwargs: calls.append(("tdx_quant", kwargs["timeframe"])) or pd.DataFrame()),
    )
    monkeypatch.setattr(
        upd.AkshareProvider,
        "fetch_bars",
        staticmethod(lambda **kwargs: calls.append(("akshare", kwargs["timeframe"])) or pd.DataFrame()),
    )
    ...
    assert calls == [("tdx_quant", "1m"), ("tdx_quant", "5m"), ("akshare", "15m")]
...
def test_main_fans_out_symbols_across_selected_timeframes(monkeypatch, capsys) -> None:
    ...
    def _fake_update_one_symbol(
        ...
        del export_excel
        calls.append((timeframe, symbol, str(local_root)))
    ...
    assert calls == [
        ("1d", "000001.SZ", str(upd.ROOT / "data" / "market" / "daily" / "1d")),
        ("1d", "000002.SZ", str(upd.ROOT / "data" / "market" / "daily" / "1d")),
        ("30m", "000001.SZ", str(upd.ROOT / "data" / "market" / "daily" / "30m")),
        ("30m", "000002.SZ", str(upd.ROOT / "data" / "market" / "daily" / "30m")),
    ]
    stdout = capsys.readouterr().out
    assert "[timeframe=1d] 开始更新，共 2 只标的" in stdout
    assert "[timeframe=1d] 更新完成：success=1, failed=1, total=2" in stdout
    assert "[timeframe=30m] 开始更新，共 2 只标的" in stdout
    assert "[timeframe=30m] 更新完成：success=1, failed=1, total=2" in stdout
    ...
    monkeypatch.setattr(
        ...
        staticmethod(
            lambda **kwargs: pd.DataFrame(
                columns=pd.Index(["date", "symbol", "open", "high", "low", "close", "volume", "amount"])
            )
        ),
    )
    monkeypatch.setattr(upd.AkshareProvider, "to_standard_symbol", staticmethod(lambda x: "000001.SZ"))
...
def test_update_one_symbol_uses_tdx_quant_provider_for_1m(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        upd.AkshareProvider,
        "fetch_bars",
        staticmethod(lambda **kwargs: (_ for _ in ()).throw(AssertionError("akshare should not be used for 1m"))),
    )

    local_root = tmp_path / "data" / "market" / "1m"
    upd.update_one_symbol("000001.SZ", "2024-01-01", "2024-01-02", "qfq", local_root, export_excel=False, timeframe="1m")
...
def test_update_one_symbol_preserves_root_cause_in_error_message(tmp_path: Path, monkeypatch):
    import pytest
